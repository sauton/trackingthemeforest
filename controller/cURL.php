...
class cURLtheme extends base_curl {
	...
	public function arrange_category() {
		$arr_cat = json_decode( self::CAT_CACHED );
	...
	public function getWeeklyTopTen( $arr_nitches = [], $top = 10 ) {
		...
		foreach ( $arr_nitches as $item ) {
			...
			$url_crawler = self::URL_WEEKLY_SOURCE . urlencode( substr( $item, 1 ) );
//			print_r( $url_crawler );
...
//					echo '</pre>';
	...
	public function getlisttoprank( $arr_nitches = [], $sortby = 'sales', $date = '', $top = 5 ) {
		...
		foreach ( $arr_nitches as $item ) {
			$url_crawler = self::URL_SOURCE_CAT . $item . $sortby . $date;
//			print_r( $url_crawler );
			$this->domHTML( $url_crawler );
...
//				$this->arr_cat[ substr( $link->getAttribute( 'href' ), 9 ) ] = $link->textContent;
	...
	public function getAjaxDetailTheme( $themes ) {
		?>
        ...
        <table class="table-bordered">
			...
			<?php
			$this->getRowKeyTable( self::ARRAY_DEF );
			foreach ( $themes as $value_url_theme ):
	...
	public function getAjaxWeeklySale( $themes ) {
	    echo 'ok';
	    die;
		?>
        ...
        <table class="table-bordered">
			...
			<?php
			if ( ! empty( $this->array_weekly_row ) ) {
				$this->getRowKeyTable( $this->array_weekly_row );

				$arr_top = $this->getWeeklyTopTen( $themes );
				//			echo '<pre>';
				//			print_r( $_GET['themes'] );
				//			print_r( $arr_top );
				//			echo '</pre>';
	...
	public function getAjaxTopCategory( $theme, $sort, $date, $top ) {
		...
		if ( TRUE && ! empty( $array_rank_url_detail ) ):?>
            ...
            <table class="table-bordered">
				...
				foreach ( $array_rank_url_detail as $key => $val ) {
					$this->getRowNitchesTable( self::ARRAY_DEF, $key );
